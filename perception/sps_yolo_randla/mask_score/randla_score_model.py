...
class RandLANet(nn.Module):
    ...
    def forward(self, input):
        ...
        # scores shape (B, num_classes, N, 1)
        scores = scores.squeeze(-1).permute(0, 2, 1)  # shape (B, N, num_classes)
        masked_scores = scores * (input[...,-1].unsqueeze(-1))
        final_scores = torch.mean(masked_scores.reshape(B, -1), dim=-1, keepdim=True)
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    # print(pred)
    print(t1-t0, cloud.shape, pred.shape)
