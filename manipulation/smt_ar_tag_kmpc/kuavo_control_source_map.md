...
This file maps the robot-control components used by the SMT manipulation
pipeline to the pinned Kuavo ROS application submodule. The submodule is checked
out under `kuavo_ros_application/kuavo` and provides the reusable robot-control
infrastructure. The paper-specific task logic remains in `manipulation/` and
uses these upstream controllers through ROS topics and services.
...
| AR-tag or ArUco detection support | `kuavo_ros_application/kuavo/src/hand_eye_calibration/aruco_ros/` and `kuavo_ros_application/kuavo/src/kuavo_assets/models/apriltag/` | Provides tag detection, tag messages, and simulation assets used by the SMT tag localization path. |
...
The Kuavo ROS application submodule is an upstream robot-control dependency. The
paper-specific release documents how the SMT task connects to it:
...
Deployment-only values such as production rack coordinates, operator account
names, private image locations, and factory calibration constants are not part
of this source map. Reproducible deployments should replace those values with a
new local calibration.
